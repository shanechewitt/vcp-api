...
class MarylandTaxCreditService {
  ...
  public async getMarylandTaxCredits(res: Response): Promise<void> {
    ...
    const data = await page.evaluate(() => {
        ...
        rows.map(row => {
          const type = row.querySelector('td[data-field-key="field_22"]')?.innerHTML.replace( /(<([^>]+)>)/ig, '').trim();
          console.log(type);
          if (type === 'Tax Credit') {
            taxCredits.push(<TaxCreditSummary>{
                name: row.querySelector('td[data-field-key="field_21"]')?.innerHTML.replace( /(<([^>]+)>)/ig, '').trim(), 
                details: row.querySelector('td[data-field-key="field_23"]')?.innerHTML.replace( /(<([^>]+)>)/ig, '').trim()
            }
            )
          }
        });
  ...
  public async createExcelFile(marylandTaxSummaries: TaxCreditSummary[], res: Response): Promise<void> {
